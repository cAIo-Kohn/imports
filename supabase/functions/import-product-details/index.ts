...
serve(async (req) => {
  ...
  try {
    ...
    const { products } = await req.json() as { products: ProductDetails[] };
    ...
    const productsToUpdate: (ProductDetails & { id: string })[] = [];
    ...
    console.log(`Products to update: ${productsToUpdate.length}, Products ignored: ${ignoredProducts.length}`);
    ...
    for (let i = 0; i < productsToUpdate.length; i += BATCH_SIZE) {
      ...
      if (upsertError) {
        console.error(`Error updating batch ${i / BATCH_SIZE + 1}:`, upsertError);
        errors.push(`Batch ${i / BATCH_SIZE + 1}: ${upsertError.message}`);
      } else {
        updatedCount += batch.length;
      }

      console.log(`Processed batch ${i / BATCH_SIZE + 1}, updated: ${updatedCount}`);
    }
    ...
    console.error('Error in import-product-details:', error);
    return new Response(
