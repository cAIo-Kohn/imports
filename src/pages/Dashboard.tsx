...
import { DevelopmentItem, DevelopmentCardStatus } from './Development';
...
export default function Dashboard() {
  const { user } = useAuth();
  const { isBuyer, isQuality, isMarketing, isTrader } = useUserRole();
  ...
  // Centralized hook: shared cache with Development page, includes realtime
  const { data: items = [], isLoading: itemsLoading } = useDevelopmentItems();
  ...
  // Real-time subscription for Dashboard-specific queries only
  // (development-items realtime is handled by useDevelopmentItems hook)
  const dashboardInvalidateRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    const handleDashboardInvalidate = () => {
      if (dashboardInvalidateRef.current) clearTimeout(dashboardInvalidateRef.current);
      dashboardInvalidateRef.current = setTimeout(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'new_product_approvals' }, handleDashboardInvalidate)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'purchase_orders' }, handleDashboardInvalidate)
      .subscribe();
